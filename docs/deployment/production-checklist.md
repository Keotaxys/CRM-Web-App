...
14. With explicit migration approval, set `ALLOW_PRODUCTION_MIGRATION=crm-web-app-97b91` and run `node scripts/migrate.mjs --apply --project crm-web-app-97b91 --confirm-project crm-web-app-97b91`. Retain the aggregate report and any private claim-remediation artifact.
...
node scripts/migrate.mjs --apply --project crm-web-app-97b91 --confirm-project crm-web-app-97b91
...
Its apply mode additionally requires `--apply`, matching `--confirm-project`, and `ALLOW_PRODUCTION_MIGRATION=crm-web-app-97b91`. Do not apply it unless an approved rollback requires exact claim restoration.
...
Restore the recorded previous Rules source, not an assumed local file. For this baseline, use `artifacts/pre-upgrade-baseline-2026-08-25/firestore.rules` and verify its recorded ruleset ID above. Deploy it through a rollback-specific Firebase config that points `firestore.rules` to that captured file, then re-run the actor matrix.

### Storage Rules and CORS

Restore Storage Rules from `artifacts/pre-upgrade-baseline-2026-08-25/storage.rules` through a rollback-specific Firebase config and verify the recorded ruleset ID. Restore CORS from `artifacts/private/storage-cors-before.json` with the exact bucket command above. Rules rollback does not restore CORS, and CORS rollback does not restore Rules; verify both separately.
...
Restore Auth Custom Claims separately from the verified snapshot artifact because Firestore import does not restore them. Run the restore dry-run, inspect the UID-only plan, then use the guarded apply only if approved. Force affected users to refresh tokens afterward and verify claim/profile consistency.

...
async function deleteAbandonedManagedImages(bucket, id, customer) {
  ...
  const paths = slots
    .filter(([field, path]) => customer[field] !== path)
    .map(([, path]) => path);
...
export async function archiveCustomerOperation({ db }, actor, data) {
  ...
  if (customer.recordState !== 'active') {
    throw new Error('Only active customer can be archived');
  }
...
export async function trashCustomerOperation({ db }, actor, data) {
  const ref = db.doc(`customers/${data?.id}`);
  const snapshot = await ref.get();
  ...
  if (!canTrashRecord(actor, snapshot.data())) {
    throw new Error('Customer trash denied');
  }
  ...
  await ref.update({
    ...
    deletedBy: actor.uid,
    deletedAt,
    purgeAfter: Timestamp.fromDate(
      trashExpiresAt(deletedAt),
    ),
    updatedBy: actor.uid,
    updatedAt: FieldValue.serverTimestamp(),
  });
...
export async function transferCustomerOperation(
  { db },
  actor,
  data,
) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const related = await transaction.get(
      db
        .collection('activities')
    ...
    if (!related.empty) {
      throw new Error(
        'Resolve related activities before transferring this customer',
      );
    }

    transaction.update(ref, {
      branchId: data.branchId,
      branch: branchLabel(data.branchId),
      updatedBy: actor.uid,
      updatedAt: FieldValue.serverTimestamp(),
    });
  });
...
export async function restoreCustomerOperation(
  { db },
  actor,
  data,
) {
  ...
  await db.doc(`customers/${data?.id}`).update({
    recordState: 'active',
    deletedBy: null,
    deletedAt: null,
    purgeAfter: null,
...
async function deleteManagedImages(
  bucket,
  id,
  customer,
) {
  ...
  const paths = new Set(
    [
      `customers/${id}/customer-photo`,
      `customers/${id}/place-photo`,
...
export async function permanentlyDeleteCustomerOperation(
  ...
  if (customer.recordState !== 'trashed') {
    throw new Error(
      'Only trashed customer can be permanently deleted',
    );
  }

  if (await hasRelatedActivities(db, ref.id)) {
    throw new Error(
      'Resolve related activities before permanently deleting this customer',
    );
  }
...
export async function cleanupExpiredCustomers(
  ...
  const snapshots = await db
    ...
    .where('recordState', '==', 'trashed')
    .get();

  const expired = snapshots.docs.filter((item) =>
    isTrashExpired(
      item.data().deletedAt,
      now,
    ),
  ...
  for (const item of expired) {
    ...
    if (await hasRelatedActivities(db, item.id)) {
      continue;
    }
...
export async function abortCustomerUploadsOperation(
  { db, bucket },
  actor,
  data,
) {
  ...
  return {
    id: ref.id,
    deletedObjects:
      await deleteAbandonedManagedImages(
        bucket,
        ref.id,
        snapshot.data(),
      ),
...
const CUSTOMER_PROGRESS_STATUS =
  'ດຳເນີນການແລ້ວ';

const SYNCABLE_ACTIVITY_STATUSES =
  new Set([
    'planned',
    'confirmed',
  ]);

const CUSTOMER_TIME_ZONE = 'Asia/Vientiane';

const laoDateFormatter =
...
function laoCalendarDateKey(value) {
  ...
  const parts =
    laoDateFormatter.formatToParts(date);
...
function isCurrentCustomerVisit(
  activity,
  customer,
  now,
) {
  ...
  if (
    activity.branchId !== customer.branchId
  ) {
    return false;
  }

  if (
    activity.recordState !== 'active'
  ) {
    return false;
  }
  ...
  const activityDay =
    laoCalendarDateKey(startAt);

  const currentDay =
    laoCalendarDateKey(now);
...
export async function changeCustomerStatusOperation(
  { db, now: nowProvider },
  actor,
  data,
) {
  ...
  return db.runTransaction(
    async (transaction) => {
      ...
      if (
        customer.recordState
        !== 'active'
      ) {
        throw new Error(
          'Only active customer status can be changed',
        );
      }
      ...
      if (
        ...
        const syncCandidates =
          currentVisits.filter(
            (activity) =>
              SYNCABLE_ACTIVITY_STATUSES.has(
                activity.status,
              ),
          );

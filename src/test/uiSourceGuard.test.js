...
it('keeps the mobile viewport and motion safety contracts', () => {
  const tokens = readFileSync('src/styles/tokens.css', 'utf8');
  const components = readFileSync('src/styles/components.css', 'utf8');
  const screens = readFileSync('src/styles/screens.css', 'utf8');

  expect(tokens).toMatch(/@media\s*\(prefers-reduced-motion:\s*reduce\)[\s\S]*scroll-behavior:\s*auto\s*!important/);
  expect(components).toMatch(/\.ui-button,[\s\S]*min-height:\s*var\(--touch-target\)[\s\S]*min-width:\s*var\(--touch-target\)/);

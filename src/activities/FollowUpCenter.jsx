...
export default function FollowUpCenter() {
  ...
  return <><Navbar title="ສູນຕິດຕາມຕໍ່"/><main className="page-content"><div className="summary-grid">{Object.values(FOLLOW_UP_BUCKETS).map((item) => <button key={item} className={`summary-card ${bucket === item ? 'active' : ''}`} onClick={() => setBucket(item)}><span>{followUpBucketLabel(item)}</span><strong>{counts[item]}</strong></button>)}</div><section className="activity-list mt-5">{shown.map((item) => <ActivityCard activity={item} key={item.id}/>)}{!shown.length && <div className="page-state">ບໍ່ມີວຽກຕິດຕາມໃນກຸ່ມນີ້</div>}</section></main></>;
}

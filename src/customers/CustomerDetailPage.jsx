...
export default function CustomerDetailPage() {
  ...
  return <><Navbar title={customer.name} showBack/><main className="page-content narrow space-y-4">{actionError && <div className="error-banner">{actionError}</div>}
    <section className="panel customer-profile"><div className="detail-photos"><div><ManagedImage storagePath={customer.imageStoragePath} legacyUrl={customer.imageUrl} alt="Customer" fallback={<div className="image-placeholder">ບໍ່ມີຮູບ</div>}/><span>ຮູບລູກຄ້າ</span></div><div><ManagedImage storagePath={customer.placeImageStoragePath} legacyUrl={customer.placeImageUrl} alt="Place" fallback={<div className="image-placeholder">ບໍ່ມີຮູບ</div>}/><span>ຮູບຮ້ານ</span></div></div>
      ...
      <div className="flex flex-wrap gap-2 mt-4"><Link className="btn-primary" to={`/customers/${id}/edit`}>ແກ້ໄຂ</Link><Link className="btn-secondary" to={`/activities/new/customer_visit?customerId=${id}`}>ສ້າງການຢ້ຽມລູກຄ້າ</Link><button className="btn-ghost" onClick={archive}>ເກັບເຂົ້າຄັງ</button>{canTrashCustomer(actor, customer) && <button className="btn-danger" onClick={trash}>ຍ້າຍໄປຖັງຂີ້ເຫຍື້ອ</button>}</div>
    ...
    <section className="panel"><div className="section-heading"><h2>ກິດຈະກຳ & ປະຫວັດຢ້ຽມ</h2><span>{activities.length}</span></div>{activities.length ? <div className="activity-list compact">{activities.map((activity) => <Link to={`/activities/${activity.id}`} key={activity.id} className="activity-row"><div><strong>{activity.title}</strong><p>{activityTypeLabel(activity.type)} · {formatDateTime(activity.startAt)}</p></div><span className={`activity-status ${activity.status}`}>{activityStatusLabel(activity.status)}</span></Link>)}</div> : <p className="muted py-6 text-center">ຍັງບໍ່ມີກິດຈະກຳ</p>}</section>
  </main></>;
}

...
export default function GiftCorrectionSheet({ distribution, gifts = [], onClose, onSuccess }) {
  ...
  const newTotal = metadataReady ? rows.reduce((total, item) => total + giftLineTotal({ packs: item.packs, looseUnits: item.looseUnits }, giftMap[item.giftId]), 0) : null;
  ...
  const submit = async (event) => {
    event.preventDefault(); if (submittingRef.current || !metadataReady) return;
  ...
  return <ModalSheet open onClose={close} title="ແກ້ໄຂການແຈກເຄື່ອງ" description={`${distribution.branchId} · ${distribution.id}`} mobileSheet footer={<><Button variant="neutral" disabled={busy} onClick={close}>ຍົກເລີກ</Button><Button type="submit" form="gift-correction-form" busy={busy} disabled={!metadataReady}>{mode === 'cancel' ? 'ຢືນຢັນການຍົກເລີກ' : 'ຢືນຢັນການແກ້ໄຂ'}</Button></>}>
    <form id="gift-correction-form" className="form-stack" onSubmit={submit}>
      ...
      {!metadataReady ? <p role="status">ກຳລັງໂຫຼດຂໍ້ມູນເຄື່ອງແຈກ...</p> : <p>ຈຳນວນເກົ່າ: {oldTotal} · ຈຳນວນໃໝ່: {mode === 'cancel' ? 0 : newTotal}</p>}
